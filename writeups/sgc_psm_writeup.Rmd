...
In `sgc_psm_writeup.Rmd`, which uses the source code `psm_writeup_funcs.R`, descriptive statistics and visualizations are generated (code used to create this document). Each study area's matched pairs table is joined into a single table with a study area flag. Additionally, 2009 NOAH data must be crosswalked to 2010 Census boundaries. Next, outmigration block group level data from the InfoGroup dataset is read in - using the "all", "low income", "renters", and "low income renters" datasets. This data must be cleaned and summarized at the tract level for comparison.
