# Open libraries and set Parameters---------------------------------------------
library(tidyverse)
library(sf)
library(sp)
library(tigris)
...
# Files
## Matched Stats
total_stats_geo_file <- "./SGC/total_stats_geo.gpkg"

## Full Area data
full_area_info_file <- "./SGC/full_area_info.gpkg"
...
paired_geoids <- 
  ...
  mutate(
    ...
    geoid_pair = 
      if_else(
        is.na(geoid_pair), lag(GEOID, order_by = geoid_group), geoid_pair
      ),
    # add location filter
    location = 
      case_when(
...
study_data_choices <- 
  c(
    ...
    "Outmigration Renters",
    "Outmigration Low Income Renters"
  )
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    else if (input$study_data == "Outmigration All") {
      proxy %>%
        # Reset controls (need to do one by one)
        removeControl("noah_legend_clear") %>% 
        removeControl("noah_legend_clear2") %>% 
    ...
    else if (input$study_data == "Outmigration Low Income") {
      proxy %>%
        # Reset controls (need to do one by one)
        removeControl("noah_legend_clear") %>% 
        removeControl("noah_legend_clear2") %>% 
    ...
    else if (input$study_data == "Outmigration Renters") {
      proxy %>%
        # Reset controls (need to do one by one)
        removeControl("noah_legend_clear") %>% 
        removeControl("noah_legend_clear2") %>% 
    ...
    else if (input$study_data == "Outmigration Low Income Renters") {
      proxy %>%
        # Reset controls (need to do one by one)
        removeControl("noah_legend_clear") %>% 
        removeControl("noah_legend_clear2") %>% 
